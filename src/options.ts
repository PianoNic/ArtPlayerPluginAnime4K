...
export interface Anime4kOptions {
  ...
  setting?: boolean;
  /** SVG or HTML for the settings entry icon. Defaults to a sparkles glyph. */
  icon?: string;
...
/** Lucide "sparkles" (ISC licence), sized for the ArtPlayer settings row. */
...
/**
 * Turns whatever the host passed into a complete, valid option set. Never throws: a plugin that
 * breaks the player over a typo in its options is worse than one that ignores the typo.
 */
